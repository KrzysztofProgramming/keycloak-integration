...
import static me.krzysztofprogramming.userprovider.roles.CustomRoleModel.ERROR_MESSAGE;

@Slf4j
public class RolesManager implements RealmModel {
    ...
    public RolesManager(ComponentModel componentModel) {
        this.componentModel = componentModel;
    }

    private CustomRoleModel mapToCustomRole(SingleRoleResponseDto roleResponseDto) {
    ...
    public CustomRoleModel getRoleById(String id) {
        return rolesByNames.get(id);
    }

    @Override
    public RoleModel getRole(String name) {
        return getRoleById(name);
    }
    ...
    public Stream<RoleModel> getRolesStream() {
        return rolesByNames.values().stream().map(Function.identity());
    }
    ...
    public Stream<RoleModel> getRolesStream(Integer firstResult, Integer maxResults) {
        ...
        return rolesByNames.values().stream().skip(firstResult).limit(maxResults).map(Function.identity());
    }
    ...
    public Stream<RoleModel> searchForRolesStream(String search, Integer first, Integer max) {
        ...
        return rolesByNames.values().stream().filter(role -> role.getName().equalsIgnoreCase(search))
                .skip(first).limit(max).map(Function.identity());
    }
